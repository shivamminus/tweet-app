...
from modals import User_mgmt

class Signup():
    loginid = StringField('loginid',validators=[DataRequired(),Length(min=4)])
    firstname = StringField('Firstname',validators=[DataRequired(),Length(min=4)])
    lastname = StringField('Lastname',validators=[DataRequired(),Length(min=4)])
    email = StringField('Email',validators=[DataRequired(),Email()])
    password = PasswordField('Password',validators=[DataRequired(),Length(min=6)])
    contact = StringField('loginid',validators=[DataRequired(),Length(min=4,max=11)])
